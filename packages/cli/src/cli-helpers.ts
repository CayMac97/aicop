...
export function buildScanOptions(targetPath: string, config: VibescanConfig, opts: CliOptions, minSeverity: Severity): ScanOptions {
  return {
    ...
    format: (opts.format as ScanOptions['format']) ?? config.output?.format ?? 'terminal',
    output: opts.output ?? (((opts.format as ScanOptions['format'] ?? config.output?.format) === 'html') ? config.output?.htmlReportPath : undefined),
    ci: Boolean(opts.ci),
